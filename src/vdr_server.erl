...
start_link(PortVDR) ->  
	case gen_server:start_link({local, ?MODULE}, ?MODULE, [PortVDR], []) of
        {ok, Pid} ->
            {ok, Pid};
        ignore ->
            common:loginfo("vdr_server:start_link(~p) fails : ignore", [PortVDR]),
            ignore;
        {already_started, Pid} ->
            common:loginfo("vdr_server:start_link(~p) fails : already_started : ~p", [PortVDR, Pid]),
            {already_started, Pid}
    end.
...
init([PortVDR]) ->    
    ...
    case gen_tcp:listen(PortVDR, Opts) of	    
		{ok, LSock} -> 
            common:loginfo("vdr_server:init([~p]) : gen_tcp:listen ok", [PortVDR]),
		...
		{error, Reason} ->	        
            common:loginfo("vdr_server:init([~p]) : gen_tcp:listen fails : ~p", [PortVDR, Reason]),
			{stop, Reason}    
	end. 
