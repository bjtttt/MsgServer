...
-module(ti_wsock_handshake).
-include("ti_wsock.hrl").
...
open(Resource, Host, Port) ->
  ...
  Headers =[
    ...
    {"Upgrade", "websocket"},
    {"Connection", "upgrade"},
    {"Sec-Websocket-Key", ti_wsock_key:generate()},
    {"Sec-Websocket-Version", integer_to_list(?VERSION)}
  ],

  Message = ti_wsock_http:build(request, RequestLine, Headers),
...
validate_http_status(Response) ->
  "101" == ti_wsock_http:get_start_line_value(status, Response).

-spec validate_upgrade_header(Response ::#http_message{}) -> boolean().
validate_upgrade_header(Response) ->
  "websocket" == string:to_lower(ti_wsock_http:get_header_value("upgrade", Response)).

-spec validate_connection_header(Response ::#http_message{}) -> boolean().
validate_connection_header(Response) ->
  "upgrade" == string:to_lower(ti_wsock_http:get_header_value("connection", Response)).
...
validate_sec_websocket_accept_header(Response, Handshake) ->
  ClientKey         = ti_wsock_http:get_header_value("sec-websocket-key", Handshake#handshake.message),
  BinaryClientKey   = list_to_binary(ClientKey),
  ExpectedHashedKey = base64:encode_to_string(crypto:sha(<<BinaryClientKey/binary, ?GUID>>)),
  HashedKey         = ti_wsock_http:get_header_value("sec-websocket-accept", Response),
